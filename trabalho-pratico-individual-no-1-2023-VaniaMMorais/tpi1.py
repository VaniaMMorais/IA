        ...
        if self.optimize == 0:
            ...
            while self.open_nodes != []:
                ...
                for a in self.problem.domain.actions(node[0]):
                    ...
                    if newstate not in self.get_path(node):
                        newnode = (newstate,nodeID, node[2]+ cost_node,self.problem.domain.heuristic(newstate, self.problem.goal), node[4]+1)
                        lnewnodes.append(len(self.all_nodes))
                        self.all_nodes.append(newnode)
                self.add_to_open(lnewnodes)
            return None
            ...
            while self.open_nodes != []:
                ...
                for a in self.problem[0][0](node[0]):
                    ...
                    if newstate not in self.get_path(node):
                        newnode = (newstate,nodeID, node[2]+ cost_node,self.problem[0][3](newstate, self.problem[2]), node[4]+1)
                        lnewnodes.append(len(self.all_nodes))
                        self.all_nodes.append(newnode)
                self.add_to_open(lnewnodes)
            return None
